...
const CreateTournament = () => {
    ...
    // 1. State for standard tournament details
    ...
    // 2. State for the Dynamic Form Builder (Starts with one default field)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Clean up the custom fields before sending to the backend
            const formattedCustomFields = customFields.map(field => ({
                label: field.label,
                fieldType: field.fieldType,
                isRequired: field.isRequired,
                // Convert comma-separated string into an array for dropdowns/checkboxes
                options: field.optionsString ? field.optionsString.split(',').map(s => s.trim()) : []
            }));

            // Combine both states into one payload matching our Mongoose schema
            const payload = {
                ...basicDetails,
    ...
    return (
        <div className="max-w-3xl mx-auto">
            <div className="pb-4 mb-8 border-b border-gray-200">
                ...
                <p className="mt-2 text-sm text-gray-500">Define the rules and build your custom registration form.</p>
            </div>

            {error && <div className="p-4 mb-6 text-sm text-white bg-red-900 rounded-md">{error}</div>}

            <form onSubmit={handleSubmit} className="space-y-10">
                
                {/* --- SECTION 1: BASIC DETAILS --- */}
                <section className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
                    <h2 className="mb-4 text-lg font-bold text-gray-900">1. Basic Details</h2>
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="startDate" required value={basicDetails.startDate} onChange={handleBasicChange} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-gray-900 focus:outline-none" />
                        </div>

                        <div>
                            <label className="block mb-1 text-sm font-medium text-gray-700">Joining Fee (₹)</label>
                            <input type="number" min="0" name="joiningFee" value={basicDetails.joiningFee} onChange={handleBasicChange} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-gray-900 focus:outline-none" />
                        </div>
                    </div>
                </section>
                ...
                <section className="p-6 bg-gray-50 border border-gray-200 rounded-lg shadow-sm">
                    ...
                    <div className="space-y-4">
                        {customFields.map((field, index) => (
                            <div key={index} className="relative p-4 bg-white border border-gray-200 rounded-md shadow-sm group">
                                
                                {/* Delete Field Button */}
                                {customFields.length > 1 && (
                                    <button type="button" onClick={() => removeField(index)} className="absolute text-gray-400 top-2 right-2 hover:text-red-600">
                                        ✕
                                    </button>
                                )}
                                ...
                                <div className="grid grid-cols-1 gap-4 md:grid-cols-12">
                                    <div className="md:col-span-6">
                                        ...
                                        <input type="text" required value={field.label} onChange={(e) => handleFieldChange(index, 'label', e.target.value)} placeholder="e.g., In-Game ID" className="w-full px-3 py-1.5 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-gray-900 focus:outline-none" />
                                    </div>
                                    
                                    <div className="md:col-span-4">
                                        <label className="block mb-1 text-xs font-medium text-gray-700">Answer Type</label>
                                        <select value={field.fieldType} onChange={(e) => handleFieldChange(index, 'fieldType', e.target.value)} className="w-full px-3 py-1.5 text-sm bg-white border border-gray-300 rounded focus:ring-2 focus:ring-gray-900 focus:outline-none">
                                    ...
                                    <div className="flex items-end md:col-span-2 pb-1.5">
                                        <label className="flex items-center text-sm text-gray-700 cursor-pointer">
                                            <input type="checkbox" checked={field.isRequired} onChange={(e) => handleFieldChange(index, 'isRequired', e.target.checked)} className="w-4 h-4 mr-2 text-gray-900 border-gray-300 rounded focus:ring-gray-900" />
                                            Required
                                        </label>
                                    </div>
                                </div>

                                {/* Conditionally render the 'Options' input if they select dropdown */}
                                {field.fieldType === 'dropdown' && (
                                    <div className="mt-3">
                                        <label className="block mb-1 text-xs font-medium text-gray-700">Dropdown Options (Comma separated)</label>
                                        <input type="text" required value={field.optionsString} onChange={(e) => handleFieldChange(index, 'optionsString', e.target.value)} placeholder="e.g., Duelist, Initiator, Controller, Sentinel" className="w-full px-3 py-1.5 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-gray-900 focus:outline-none" />
                                    </div>
                                )}
                            </div>
                        ))}
